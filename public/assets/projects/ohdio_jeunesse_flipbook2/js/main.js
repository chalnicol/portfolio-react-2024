...
const myPanels = ['Filbuste', 'Pas Moi', 'El Kapoutchi', 'A qui la frite', 'L\'incroyable histoire'];
...
function initAd () {
    ...
    .from ('.tap-logo', { duration:0.6, opacity: 0 }, 'scene1+=1.4')

    .from ('.tap-txt-1', { duration:0.8, opacity: 0 }, 'scene1+=0.8')

    .from ('.tap-txt-2', { duration:0.8, opacity: 0 }, 'scene1+=1.1')

    .from ('.tap-view', { duration:0.8, opacity: 0 }, 'scene1+=1.8')
    
    .to ('.tap-view', { scale:0.90, duration : 0.2, yoyo:true, repeat : 3, transformOrigin:'50% 62%' }, 'scene1+=2.1')
    ...
    mc.on("panleft swipeleft tap", function(ev) {
        ...
        if ( !cardShown ) {
            ...
            if ( ev.type == 'tap') {
                ...
                //syncUp ( { exit : 'Clickthru Exit Click ' + (currentCardShown+1) });
